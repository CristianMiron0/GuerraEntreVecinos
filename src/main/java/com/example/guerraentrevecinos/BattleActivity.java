...
public class BattleActivity extends AppCompatActivity {
    ...
    // Track revealed cells (for visual feedback only, can still attack)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        selectedPower = getIntent().getStringExtra("SELECTED_POWER"); // ✅ NEW
    ...
    private void startPlayerTurn() {
        isPlayerTurn = true;
        hasAttackedThisTurn = false; // Reset attack flag at start of turn
    ...
    private void onEnemyCellClicked(int row, int col) {
        ...
        // ✅ NEW: Check if player already attacked this turn
        ...
        // ✅ Disable all enemy cells immediately to prevent multi-click
        ...
        if (hitUnit != null) {
            // HIT! Always launch mini-duel when attacking a unit
            launchMiniDuel(row, col, hitUnit.type, true); // true = player is attacking
        } else {
            // MISS - empty cell
            showMissOnEnemyGrid(row, col);

            // Still end turn after a miss
            new Handler().postDelayed(this::endPlayerTurn, 1500);
        }
    }

    // Method to enable/disable all enemy cells
    private void setEnemyGridClickable(boolean clickable) {
        for (int row = 0; row < 8; row++) {
            for (int col = 0; col < 8; col++) {
                enemyCells[row][col].setClickable(clickable);

                // Optional: Visual feedback that cells are disabled
                if (!clickable) {
                    enemyCells[row][col].setAlpha(0.5f);
                } else {
                    // Only restore alpha if not already revealed
                    if (!enemyRevealedCells[row][col]) {
                        enemyCells[row][col].setAlpha(0.8f); // Fog alpha
                    } else {
                        enemyCells[row][col].setAlpha(1f); // Revealed alpha
                    }
                }
            }
        }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_CODE_MINI_DUEL && resultCode == RESULT_OK) {
            ...
            // ✅ Get duel choices from MiniDuelActivity (need to pass these back)
            ...
            if (isPlayerAttacking) {
                ...
                // ✅ Save move
                saveMoveToDatabase(row, col, true, attackerChoice, defenderChoice, result);
    ...
    private void damagePlayerUnit(SetupActivity.UnitPosition unit) {
        ...
        unit.health--;
        lastAttackedPlayerUnit = unit; // ✅ Track for dog fear
        ...
        if (unit.health == 1) {
            ...
            // ✅ Check if cat and teleport before destruction
            if (unit.type.equals("cat") && !unit.abilityUsed) {
                boolean teleported = abilityManager.activateCatTeleport(
    ...
    private void endPlayerTurn() {
        isPlayerTurn = false;
        hasAttackedThisTurn = false; // Reset for next turn

        // Make sure enemy grid is disabled during enemy turn
        setEnemyGridClickable(false);
    ...
    private void aiTakeTurn() {
        ...
        for (SetupActivity.UnitPosition unit : playerUnits) {
            if (unit.health > 0) {
                // ✅ Exclude dog with active fear
                if (unit.type.equals("dog") && unit.dogFearActive) {
                    continue; // Skip this dog
                }
                aliveUnits.add(unit);
            }
        }
        ...
        if (aliveUnits.isEmpty()) {
            // ✅ Check if only feared dogs remain
        ...
        // ✅ Deactivate fear after this turn
        if (lastAttackedPlayerUnit != null &&
            ...
            if (lastAttackedPlayerUnit.health == 1) {
                cell.setBackgroundColor(Color.parseColor("#FFA500")); // Back to orange
            } else {
                cell.setBackgroundColor(Color.parseColor("#8FBC8F")); // Back to green
            }
        }
    ...
    private void startNextRound() {
        ...
        // Deactivate Garden Hose if it was used
    ...
    private void checkWinCondition() {
        // Check if all AI units destroyed
    ...
    private int getUnitIcon(String unitType) {
        switch (unitType) {
            ...
            case "rose":
                return R.drawable.rose_red; // Default red
            case "dog":
    ...
    private void setupPowerButtons() {
        ...
        // Set Tier 2 power icon and text
    ...
    // ✅ Nighttime Relocation: Move unit to adjacent cell
    private void handleNighttimeRelocation(SetupActivity.UnitPosition unit) {
        // Show dialog with adjacent cells
        new androidx.appcompat.app.AlertDialog.Builder(this)
        ...
        // Additional dialog for left/right
        new androidx.appcompat.app.AlertDialog.Builder(this)
                .setTitle("🌙 Move " + unit.type)
    ...
    // ✅ Fence Shield: Protect unit from next attack
    private void handleFenceShield(SetupActivity.UnitPosition unit) {
        ...
        // Visual indicator - add shield overlay
        ImageView cell = playerCells[unit.row][unit.col];
        cell.setBackgroundColor(android.graphics.Color.parseColor("#4DB6AC")); // Cyan
    ...
    private void handleFertilizer(SetupActivity.UnitPosition unit) {
        ...
        // Update visual
        ImageView cell = playerCells[unit.row][unit.col];
        cell.setBackgroundColor(android.graphics.Color.parseColor("#8FBC8F")); // Green
                ...
                if (row >= 0 && row < 8 && col >= 0 && col < 8) {
                    ...
                    if (!enemyRevealedCells[row][col]) {
                        ...
                        for (SetupActivity.UnitPosition unit : aiUnits) {
                            if (unit.row == row && unit.col == col && unit.health > 0) {
                                // Show unit
                                cell.setBackgroundColor(android.graphics.Color.parseColor("#FFE082")); // Light yellow
                                cell.setImageResource(getUnitIcon(unit.type));
                                cell.setAlpha(1f);
                                hasUnit = true;
                                break;
                        ...
                        if (!hasUnit) {
                            // Show empty
                            cell.setBackgroundColor(android.graphics.Color.parseColor("#C5E1A5")); // Light green
                            cell.setAlpha(1f);
                        }
